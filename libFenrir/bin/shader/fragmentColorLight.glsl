...
void main () {
	...
	vec4 fragment_light_colour = vec4 (Is + (Id + Ia)*1.0/pow(dFactor,2), 1.0); // Quadratic 

	//vec4 fragment_light_colour = vec4 (Is + (Id + Ia)*1.0/pow(dFactor,3), 1.0); // Cube 

	
	// base colour
	vec4 fragment_base_colour = vec4 (color, 1.0);
	
	frag_colour = mix(fragment_light_colour, fragment_base_colour, 0.5);
};